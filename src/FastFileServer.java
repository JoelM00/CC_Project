...
public class FastFileServer {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        Tupulo gateway = new Tupulo(InetAddress.getByName(args[1]),6000);
        Tupulo origem = new Tupulo(InetAddress.getByName(args[0]),s.getLocalPort());

        byte[] pacote = new byte[1024];
        byte[] pacote1 = new byte[1024];
        byte[] pacote2 = new byte[1024];

        ficheiros.put("ola.txt",13218);
        ficheiros.put("xau.txt",2026982);
        ficheiros.put("hello.txt",644719);
        ficheiros.put("json.txt",79150);
        ficheiros.put("lixo.txt",212);


        PacoteRegisto pr = new PacoteRegisto(gateway,origem,new ArrayList<>(ficheiros.keySet()));
        ...
        while (true) {
            ...
            if (pp.metaDados) {
                ...
                new Thread(new FastFileWorker(origem.endereco,pp.ficheiroPedido, pp.origem, pp.from, pp.to)).start();
            }
        }
    }
}
