...
public class GatewayWorkerFFS implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (true) {
                byte[] pacote_ = new byte[1024];
                DatagramPacket pacoteDeChunk= new DatagramPacket(pacote_, pacote_.length);

                s.receive(pacoteDeChunk);

                System.out.println("Recebi um chunk");
                PacoteChunk pc = PacoteChunk.descompactar(pacote_);
                System.out.println("Descompatei um chunk");
