...
public class Controlador {
    ...
    public Controlador(InetAddress endereco, String ficheiroPedido, RegistosFFS registosFFS, HttpWorker.PacoteFicheiro pacoteFicheiro) {
    ...
    public int trataPedido() throws IOException, InterruptedException {
        ...
        if (ffsComFicheiro.size() == 1 ) {

            System.out.println(" -> Acionada um busca por um FFS");
            new GatewayWorkerFFS(endereco,ffsComFicheiro.get(0), registosFFS, ficheiroPedido, pacoteFicheiro,0,0).run();
            ...
            Tupulo origemR = new Tupulo(endereco,r.getLocalPort());
            ...
            Thread aQueVaiEnviar = new Thread(() -> {
                try {
                    ...
                    System.out.println(" >>> Erro no pedido de metadados!");
                }
            });
            ...
            System.out.println("TAMANHO DO FICHEIRO: "+pr.tamanhoFicheiro);
            ...
            for (int i = 0; i < ffsComFicheiro.size(); i++) {
                ...
                int finalI = i;
                thrs.add(new Thread(() -> new GatewayWorkerFFS(endereco,ffsComFicheiro.get(finalI),registosFFS,ficheiroPedido,pacoteFicheiro,finalAnterior,finalOffset).run()));
            }
            if (pr.tamanhoFicheiro % ffsComFicheiro.size() != 0) {
                int finalOffset1 = offset;
                thrs.add(new Thread(() -> new GatewayWorkerFFS(endereco,ffsComFicheiro.get(ffsComFicheiro.size() - 1), registosFFS, ficheiroPedido, pacoteFicheiro, finalOffset1, pr.tamanhoFicheiro).run()));
            }
