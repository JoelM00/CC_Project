...
public class FastFileWorker implements Runnable {
    ...
    public FastFileWorker(String fileName,Tupulo destino,int from, int to) {
        ...
        try {
            ...
            if (file.exists()) {
                if ((to-from) == 0) {
                    ...
                    for (int i = 0; i <= parts; i++) {
                        long ofSet = i * maxPacote + from;//
                        long limit = ofSet + maxPacote;
                        //Tentativa de resolver bug, nao resolve!
                        if (limit > to) {
                            limit = to; //deve ser aqui! o PROBLEMA
                        }
